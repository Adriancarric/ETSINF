...
public abstract class Grafo {
    ...
    public Arista[] arbolRecubrimientoBFS() {
        ...
        while(q.esVacia()){
            ...
            for(l.inicio(); !l.esFin(); l.siguiente()){
             Adyacente a = l.recuperar();
             if(visitados[a.destino] == 0){
                 Arista arist = new Arista(u,a.destino,a.peso);
                 res[contador] = arist;
                 contador++;
                 visitados[a.destino] =1;
                 q.encolar(a.destino);
    ...
    public Arista[] kruskal() {       
        ...
        Arista[] res = new Arista[numVertices()-1];
        ColaPrioridad<Arista> qPrior = new PriorityQColaPrioridad<Arista>();
        MFSet m = new ForestMFSet(numVertices());
        qPrior = getAristaKruskal();

        int numA = 0;
        while(numA < numVertices() - 1 && !qPrior.esVacia()){
            Arista a = qPrior.eliminarMin();
            if(m.find(a.origen) != m.find(a.destino)){
                m.merge(a.origen,a.destino);
                res[numA++] = a;
            }
        }
        return (numA == numVertices()-1) ?res : null;
    }
    private ColaPrioridad<Arista> getAristaKruskal(){
        ColaPrioridad<Arista> aristas = new PriorityQColaPrioridad<Arista>();
                for (int i = 0; i < numVertices(); i++) {
            ListaConPI<Adyacente> l = adyacentesDe(i);
            for(l.inicio(); !l.esFin(); l.siguiente()){
                Adyacente a = l.recuperar();
                aristas.insertar(new Arista(i,a.destino,a.peso));
            }
        }
        return aristas;
    }
