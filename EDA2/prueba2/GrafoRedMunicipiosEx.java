...
public class GrafoRedMunicipiosEx {
    ...
    public double pesoAristasEnMST(Municipio m) {
        double res = 0;
        /* COMPLETAR */
        Arista[] aristas = gRM.kruskal();
        if(aristas==null){return -1;}
        int muni = getVertice(m);
        if(muni>numVertices()){return -2;}
        for(int i = 0; i<numAristas(); i++){
            Municipio muni2 = getMunicipio(aristas[i].getOrigen());
            if(m.equals(muni2)){res+=aristas[i].getPeso();}
          }
        return res;
    }
