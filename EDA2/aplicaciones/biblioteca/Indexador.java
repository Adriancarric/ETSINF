...
public class Indexador {
     ...
     */
    protected static String DIR_LIBROS = File.separator + "asigDSIC" + File.separator 
        + "libros" + File.separator + "TXT" + File.separator;
    ...
    protected Map<String, Indice> indices;
	...
	indices = new TablaHash<String, Indice>(tallaAprox);
		
    	while (fich.hasNext()) {
    	    String nombreLibro = fich.next();
    	    String fichLibro = dirHome + DIR_LIBROS + nombreLibro;
	...
	for(claves.inicio(); !claves.esFin(); claves.siguiente()){
	    if(claves.recuperar() != null){
	        res.insertar(claves.recuperar());
	       }
	   }
