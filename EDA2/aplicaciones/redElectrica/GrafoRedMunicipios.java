...
public class GrafoRedMunicipios {
    ...
    public double crearAdyKruskal() {      
        ...
        Arista[] gRMKruskal = gRM.kruskal();
        
        // PASO 2. Si kruskal devuelve null, devolver -1.0
        // COMPLETAR
        
        if(gRMKruskal == null) {return -1.0;}
        ...
        for(int i = 0; i < numVertices(); i++) {
            Arista a = gRMKruskal[i];
            coste += a.getPeso();
            
            Municipio origen = getMunicipio(a.getOrigen());
            Municipio destino = getMunicipio(a.getDestino());
            
            if(adyKruskal.recuperar(origen) == null){
            ListaConPI<Municipio> l = null;
            l.insertar(destino);
            adyKruskal.insertar(origen, l);
            } else{
                ListaConPI<Municipio> l = adyKruskal.recuperar(origen);
                l.insertar(destino);
                adyKruskal.insertar(origen, l);
            }
