...
public class CrearActuacionJDialog extends JDialog {
	...
	public CrearActuacionJDialog(Controlador c) {
		...
		{
			JLabel lblCodigo = new JLabel("Pel\u00EDcula:");
		...
		{
			JLabel lblLocalidad = new JLabel("Papel:");
		...
		comboBoxPapel.addItem("Principal");
		comboBoxPapel.addItem("Secundario");
		{
		...
		}
		{
			...
			{
				JButton cancelarButton = new JButton("Cerrar");
	...
	private void crearActuacionMenuItemActionPerformed(ActionEvent arg0){
		...
		if (pelicula==null){
			JOptionPane.showMessageDialog(this, "Se debe elegir una película.", "Error", JOptionPane.ERROR_MESSAGE);
			comboBoxPelicula.grabFocus();
			return;
		}
		if (actor==null){
			JOptionPane.showMessageDialog(this, "Se debe elegir un actor.", "Error", JOptionPane.ERROR_MESSAGE);
			comboBoxActor.grabFocus();
			return;
		}
		...
		try{
			...
			JOptionPane.showMessageDialog(this,"Actuación creada correctamente.", "Película",JOptionPane.INFORMATION_MESSAGE);
