 ...
 * lo que es cierto si se ha generado con la funci�n inventa */
...
int main(int argc, char *argv[])
{
  ...
  MPI_Scatter(v, n, MPI_DOUBLE, vloc, n, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  ...
  for (iter = 1; iter <= num_iter; iter++) {
    ...
    MPI_Allgather(xloc, n, MPI_DOUBLE, x, n, MPI_DOUBLE, MPI_COMM_WORLD);
    /*if (me == 0) {
