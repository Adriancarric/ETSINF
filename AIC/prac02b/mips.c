...

{
    ...
    default: /* el resto de instrucciones */
        ...
        case cortocircuito:
            ...
            */
            if(hay_destino_WB() && hay_fuente1_EX && MEM_WB.IR.Rdestino == ID_EX.IR.Rfuente1)
                result = wb;
            
            if(hay_destino_MEM() && hay_fuente1_EX && EX_MEM.IR.Rdestino == ID_EX.IR.Rfuente1)
                result = mem;
...

{
    ...
    case parada:
        ...
        if (hay_destino_EX() && ((hay_fuente1_ID() && IF_ID.IR.Rfuente1 == ID_EX.IR.Rdestino) || (hay_fuente2_ID() && IF_ID.IR.Rfuente2 == ID_EX.IR.Rdestino)))
        {
            IDstall = SI;
            IFstall = SI;
        }	
        ...
        if (hay_destino_MEM() && ((hay_fuente1_ID() && IF_ID.IR.Rfuente1 == EX_MEM.IR.Rdestino) || (hay_fuente2_ID() && IF_ID.IR.Rfuente2 == EX_MEM.IR.Rdestino)))
        {
            IDstall = SI;
            IFstall = SI;
        }
    ...
    case cortocircuito:
        ...
        if(hay_destino_EX() && ID_EX.IR.codop == OP_LW && 
            ((hay_fuente1_ID() && IF_ID.IR.Rfuente1 == ID_EX.IR.Rdestino) ||
            (hay_fuente2_ID() && IF_ID.IR.Rfuente2 == ID_EX.IR.Rdestino))) 
        {
         IDstall = SI;
         IFstall = SI;
        }
...

{
    ...
    case stall:
        ...
        {
            if (EX_MEM.cond) 
            { /* salto que sí salta  -> cancelamos inst en IF */
                IFnop=SI;
