                    ...
                    if (ref_iter->tipo_memoria != tipo_memoria) {
                        ...
                        switch (ref_iter->tipo_memoria) {
                            ...
                            case m_instruc:
                                ...
                                instruccion.inmediato = direccion - ref_iter->direccion - 4;
                                break;
                            default:
                                yyerror("PÁNICO");
                        } /* endswitch */
    ...
    for (i = 0; i < num_etiq; i++) /* Mira si la etiqueta existe */ {
        if (!strcmp(LE[i].texto, texto)) { /* Existe una referencia */
            if (LE[i].sin_resolver == NULL) {
                ...
                /*** Añade una referencia sin resolver a la cola de referencias */
...
void te_etiqueta(
    ...
    if (f_etiquetas) {
        ...
        for (i = 0; i < num_etiq; i++) { /* Mira si la dirección existe */
            if (LE[i].tipo_memoria == tipo_memoria) {
                if (LE[i].direccion == direccion) {
                    etiqueta = i;
                    encontrada = SI;
                } else if (!encontrada && LE[i].direccion >= base && LE[i].direccion <= direccion) {
                    etiqueta = i;
                    base = LE[i].direccion;
                }
            } /* endif */
        ...
        if (encontrada) {
            strcpy(texto, LE[etiqueta].texto);
        } else if (etiqueta >= 0) {
