...
static void imprime_estado_predictor_n
(
        char *nom,
        int e,
        int n_bits
...
static void imprime_estado_selector_n
(
        char *nom,
        int e
        ) {
...
static void imprime_tipo_predictor
(
        char * aux
        ) {
    /*************************************/
    ...
    switch (tipo_predictor) {
        case BTB_1_BIT:
            ...
            break;
        case BTB_2_BIT_H:
            sprintf(aux, "2 bit con histeresis");
            break;
        case BTB_2_BIT_S:
            sprintf(aux, "2 bit con saturacion");
            break;
        case BTB_n_BIT_H:
...
(
    ...
    fprintf(f_salida,
            "<html><head><title>mips-especulacion: %s. Ciclo %lu</title>\n",
            nombre_fich, Ciclo);
    fprintf(f_salida,
            "<style type=\"text/css\">\n"
            " td .address {\n"
            "     text-align: left; white-space: nowrap; padding: 0 0.4em;\n"
    ...
    fprintf(f_salida, "  <td width=\"75\" aling='center'>Datos</td>\n");
    fprintf(f_salida, " </tr>\n");
    ...
    for (i = SegDatos.inicio; i < SegDatos.final; i += sizeof (double)) {
        ...
        fprintf(f_salida, "  <td class='address' style='color: %s'>%s</td>\n", color, aux);
...
(
    ...
    fprintf(f_salida, "  <td>%d (%d%%)</td>\n", max_util.TAMPON_LECT, (max_util.TAMPON_LECT * 100 / TAM_TAMPON_LECT));
    fprintf(f_salida, "  <td>%d (%d%%)</td>\n", max_util.TAMPON_ESCR, (max_util.TAMPON_ESCR * 100 / TAM_TAMPON_ESCR));
    ...
    fprintf(f_salida, "  <td>%d (%d%%)</td>\n", max_util.OP_DIRECCIONES, (max_util.OP_DIRECCIONES * 100 / NUM_OPER_DIRECCIONES));
...
(
    ...
    for (s = INICIO_TAMPON_LECT;
        ...
        if (TL[s].ocupado || (!TL[s].ocupado && TL[s].orden > 0)) {
            ...
            FIRMA_2_3("RS%d.desp", s, F_DWORD " (%s)", TL[s].desplazamiento, TL[s].etiqueta);
            fprintf(f_salida, "  <td align=\"center\">%s</td>\n", vstr);
    ...
    for (s = INICIO_TAMPON_ESCR;
        ...
        if (TE[s].ocupado || (!TE[s].ocupado && TE[s].orden > 0)) {
            ...
            FIRMA_2_3("RS%d.desp", s, F_DWORD " (%s)", TE[s].desplazamiento, TE[s].etiqueta);
            fprintf(f_salida, "  <td align=\"center\">%s</td>\n", vstr);
...
(
    ...
    for (x = 0; x < MAX(MAX_CICLOS_INIC, maxima_columna_visible); x++)
        fprintf(f_salida, "  <th width=\"25\" class=\"sticky\"><a href=\"crono%03ld.html\">%lu</a></th>\n",
...
(
    ...
    if (inst_branch(IF_ISS_1.IR.codop)) {
        ...
        fprintf(f_salida, "<p>IF: PC= %llu; Instr=%s; Prediccion=%d\n", IF_ISS_1.PC, aux, IF_ISS_1.prediccion);
    }

    if (inst_branch(RB[RB_inicio].OP) && (RB[RB_inicio].estado == WB)) {
        imprime_instruccion_color(aux, RB[RB_inicio].PC, NO);
        fprintf(f_salida, "<p>&rarr;C: PC=%llu; Instr=%s, Condicion=%lld\n", RB[RB_inicio].PC, aux, RB[RB_inicio].valor.int_d);
    }
    ...
    if ((tipo_predictor == CLARIVIDENTE) || (tipo_predictor == BTB_1_BIT) ||
        ...
        fprintf(f_salida, "<table>\n");
        fprintf(f_salida, " <tr bgcolor=\"#ccddff\">\n");
        fprintf(f_salida, "  <td width=\"150\">Mascara global: </td>\n");
        fprintf(f_salida, " </tr>\n");
        fprintf(f_salida, " <tr bgcolor=\"#eeeeee\">\n");
        ...
        for (i = 0; i < TAM_BUFFER_PREDIC; i++) {
            ...
            if (BTB[i].orden > 0) { /* Entrada ocupada */
                ...
                if ((tipo_predictor != GLOBAL_n_BIT_S) && (tipo_predictor != GLOBAL_n_BIT_H)) {
                    for (j = 0; j < pwrtwo(n_bits_branch_mask_local); j++) {
                        imprime_estado_predictor_n(aux, BTB[i].v_estado[j], n_bits_pred_local);
                ...
                /* Selector. Se muestra en el predictor híbrido */
                if (tipo_predictor == HYBRID_n_BIT_S) {
                    imprime_estado_selector_n(aux, BTB[i].estado_selector);
        ...
        if ((tipo_predictor == GLOBAL_n_BIT_H) || (tipo_predictor == GLOBAL_n_BIT_S) ||
            ...
            fprintf(f_salida, " <tr bgcolor=\"#eeeeee\">\n");
            for (j = 0; j < pwrtwo(n_bits_branch_mask_global); j++) {
                imprime_estado_predictor_n(aux, Global[j], n_bits_pred_global);
...
void imprime_estadisticas() {
    ...
    PRINT_ESTAT("RS Carga:   ", TAMPON_LECT);
    PRINT_ESTAT("RS Almac.:  ", TAMPON_ESCR);
