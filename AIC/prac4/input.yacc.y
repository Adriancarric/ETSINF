...
%token <ivalue> DE_DATA DE_TEXT DE_SPACE DE_IREG DE_FREG DE_ASCIIZ DE_BTB DE_PC
...
T_INT_LIT
{
 	sprintf(desplazamiento,"%lld",$1);
    $$ = $1;}

| T_ALPHANUM
{
	hay_valor=0;
	sprintf(desplazamiento,"%s",$1);
...
| FORM_INM_L REG_INT ',' r_inmediato '(' REG_INT ')'
{
    ...
    instruccion.inmediato = $4;
     sprintf(instruccion.etiqueta, "%s", desplazamiento);
    escribe_mem_instruc(pos_instruc, instruccion);
    pos_instruc += 4;}
